      ...
      <div class="container-fluid">
        ...
        <div class="card mb-3">
          ...
          <div class="card-body">
            ...
            <form action="admin.modify?flag=${param.flag }" method="post" enctype="multipart/form-data">
	...
	<input type="hidden" name="originalfile" value="${dto.attachedfile }" />
			...
			<%if(!(request.getParameter("flag").equals("free"))){%>
             ...
             <%} %>
                ...
                <tbody>
                  <tr>
                  	<td><input type="hidden" name="idx" value="${dto.idx}">${dto.idx}</td>
                    <td><input type="hidden" name="id" value="${dto.id}">${dto.id}</td>
                    <td><input type="pass" name="pass" value="${dto.pass}">${dto.pass}</td>
                    <td><input type="text" name="title" value="${dto.title}"></td>
                    <td><input type="text" name="postdate" value="${dto.postdate}"> </td>
                    <td><input type="text" name="visitcount" value="${dto.visitcount}"></td>
                    <td><input type="text" name="content" value="${dto.content}"></td>
                <%if(!(request.getParameter("flag").equals("free"))){%>
					<td><input type="file" class="form-control" name="attachedfile" /></td>
				<%} %>
                  </tr>
                </tbody>
              </table>
              <div class="form-row float-right">
      <button class="btn btn-primary" id="advanced-set-submit">수정하기</button>
  </div>
                </form>
            </div>
          </div>
